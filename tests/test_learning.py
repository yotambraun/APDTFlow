...
def test_model_learns_synthetic_signal():
    ...
    mu, sd = s[:480].mean(), s[:480].std(); z = (s - mu) / sd
    ...
    for _ in range(50):
        tot = 0.0
        for xb, yb in loader:
            ts = torch.linspace(0, 1, T_in); opt.zero_grad()
            p, _ = m(xb, ts)
...
def test_checkpoint_roundtrip(tmp_path):
    torch.manual_seed(0)
    m = _make_model(); m.eval()
    x = torch.randn(2, 1, 24); t = torch.linspace(0, 1, 24)
    with torch.no_grad(): p1, _ = m(x, t)
    path = tmp_path / "m.pt"
    torch.save(m.state_dict(), path)
    m2 = _make_model(); m2.load_state_dict(torch.load(path)); m2.eval()
    with torch.no_grad(): p2, _ = m2(x, t)
