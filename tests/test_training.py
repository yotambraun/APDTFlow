...
def test_training_loss_decreases(dataloaders):
    ...
    final_loss = train_on_split(model, train_loader, val_loader, num_epochs=5, learning_rate=0.001, device=device)
    assert final_loss < initial_loss, f"Expected final loss {final_loss} to be lower than initial loss {initial_loss}"
...
def test_no_nan_in_parameters(dataloaders):
    ...
    model.to(device)
    train_on_split(model, train_loader, val_loader, num_epochs=5, learning_rate=0.001, device=device)
    for param in model.parameters():
        assert not torch.isnan(param).any(), "Found NaN in model parameters after training"
...
def test_checkpoint_save_and_load(tmp_path, dataloaders):
    ...
    model.to(device)
    train_on_split(model, train_loader, val_loader, num_epochs=1, learning_rate=0.001, device=device)
    ...
    for p1, p2 in zip(model.parameters(), model_new.parameters()):
        assert torch.allclose(p1, p2), "Model parameters do not match after loading checkpoint"
