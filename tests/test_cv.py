...
def test_rolling_window_splits():
    ...
    factory = TimeSeriesCVFactory(dataset, method="rolling", train_size=40, val_size=10, step_size=10)
...
def test_expanding_window_splits():
    ...
    factory = TimeSeriesCVFactory(dataset, method="expanding", train_size=20, val_size=5, step_size=5)
    splits = factory.get_splits(max_splits=2)
    assert len(splits) == 2

def test_blocked_splits():
    dataset = DummyDataset(length=50)
    factory = TimeSeriesCVFactory(dataset, method="blocked", train_size=30, val_size=10, step_size=5)
...
def test_invalid_method():
    ...
    with pytest.raises(ValueError):
        TimeSeriesCVFactory(dataset, method="invalid", train_size=30, val_size=5, step_size=5)
